...
def exchange_tensor_meta_data(io_engine_obj:IOEngine,tensor,exchange_ip,role):
    ...
    if role == "kv_provider":
        ...
        import time
        time.sleep(10)
        print(f"start send tensor meta{port}")
        ...
        import time
        print(f"start recv tensor meta{port}")
        provider_mem_metadata = MemoryDesc.unpack(receive_data(port))
        
        mem =  io_engine_obj.register_torch_tensor(tensor)
        
        print(f"!!!!!!!mem {mem}")
        
        print(f"!!!!provider_mem_metadata {provider_mem_metadata}")
        session = io_engine_obj.create_session(mem, provider_mem_metadata)
        port += 1
        ...
        return consumer_mem_metadata,provider_mem_metadata,session
...
def recv_kv_cache():
    ...
    local_mem_meta_data,remote_mem_meta_data, session = exchange_tensor_meta_data(kv_consumer,tensor,kv_provider_ip,"kv_consumer")
    ...
    for size in range(1,65):
        for idx in range(1024):
            ...
            transfer_status=session.read(0,0,1024*size,kv_consumer.allocate_transfer_uid())
            # transfer_status = kv_consumer.read(
            #     local_mem_meta_data, 0, 
            #     remote_mem_meta_data, 0, 
            #     1024*size,
